...
def dice_counts(dice):
    """Make a dictionary of how many values there are in the dice
...
def small_straight(dice):
    ...
    This function works with sets or lists or other collection types:
...
def yatzy(dice):
    """
    Score the given role in the 'Yatzy' category
...
def full_house(dice):
    """
    Score the given role in the 'Full House' category
...
def scores_in_categories(dice, categories=ALL_CATEGORIES):
    ...
    scores = [(category(dice), category)
                for category in categories
                    if category(dice) > 0]
    return sorted(scores, reverse=True, key=operator.itemgetter(0))
